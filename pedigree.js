...
(function(io, $, undefined) {
	...
	io.get_printable_svg = function(opts) {
		var local_dataset = window.pedcache.current(opts); // get current dataset
	...
	io.save = function(opts){
		var content = JSON.stringify(window.pedcache.current(opts));
		io.save_file(opts, content);
	};

	io.save_canrisk = function(opts, meta){
		var content = run_prediction.get_pedigree(window.pedcache.current(opts), undefined, meta);
		io.save_file(opts, content);
	};
...
(function(pedigree_util, $, undefined) {
	...
	pedigree_util.node_attr = function(opts, name, keys, value){
		var newdataset = ptree.copy_dataset(window.pedcache.current(opts));
	...
	pedigree_util.delete_node_by_name = function(opts, name){
		...
		var newdataset = ptree.copy_dataset(window.pedcache.current(opts));
		var node = pedigree_util.getNodeByName(window.pedcache.current(opts), name);
		if(!node){
	...
	pedigree_util.exists = function(opts, name){
		return pedigree_util.getNodeByName(window.pedcache.current(opts), name) !== undefined;
	}
...
(function(ptree, $, undefined) {
	...
	ptree.build = function(options) {
        ...
        if ( $( "#fullscreen" ).length === 0 ) {
			...
			window.pbuttons.add(opts);
			window.io.add(opts);
        }

        if(window.pedcache.nstore(opts) == -1)
        	window.pedcache.add(opts);

        window.pbuttons.updateButtons(opts);
		...
		var xytransform = window.pedcache.getposition(opts);  // cached position
...
/*		var warn = node.filter(function (d) {
		...
		function zoomFn() {
			...
			if(t.k == 1) {
				window.pedcache.setposition(opts, pos[0], pos[1]);
			} else {
				window.pedcache.setposition(opts, pos[0], pos[1], t.k);
			}
			...
			for(var p=0; p<opts.dataset.length; p++) {
				if(!p.hidden) {
					if(opts.dataset[p].mother || opts.dataset[p].father) {
						...
						var midx = window.pedigree_util.getIdxByName(opts.dataset, mother);
	...
	function get_svg_dimensions(opts) {
        return {'width' : (window.pbuttons.is_fullscreen()? window.innerWidth  : opts.width),
        	    'height': (window.pbuttons.is_fullscreen()? window.innerHeight : opts.height)};
	}
...
(function(pedigree_form, $, undefined) {
	...
	pedigree_form.update = function(opts) {
		...
		$('#id_proband, #id_exclude').click(function(e) {
			var dataset = window.pedcache.current(opts);
	...
	$(document).on('fhChange', function(e, opts){
		try {
			...
			var node = pedigree_util.getNodeByName(window.pedcache.current(opts), id)
			if(node === undefined)
...
(function(pbuttons, $, undefined) {
	...
	function click(opts) {
	    ...
	    $(document).on('webkitfullscreenchange mozfullscreenchange fullscreenchange MSFullscreenChange', function(e)  {
			var local_dataset = window.pedcache.current(opts);
	    	if (local_dataset !== undefined && local_dataset !== null) {
	    		opts.dataset = local_dataset;
	    	}
			ptree.rebuild(opts);
		...
		$( "#"+opts.btn_target ).on( "click", function(e) {
			...
			if($(e.target).hasClass('fa-undo')) {
				opts.dataset = window.pedcache.previous(opts);
				$("#"+opts.targetDiv).empty();
				ptree.build(opts);
			} else if ($(e.target).hasClass('fa-repeat')) {
				opts.dataset = window.pedcache.next(opts);
				$("#"+opts.targetDiv).empty();
				ptree.build(opts);
	...
	pbuttons.reset = function(opts, keep_proband) {
		if(keep_proband) {
			...
			window.pedcache.clear_pedigree_data(opts)
		} else {
			var proband = {
				"name":"ch1","sex":"F","mother":"f21","father":"m21","proband":true,"status":"0","display_name":"me"
			};
			window.pedcache.clear(opts); // clear all storage data
		}
	...
	pbuttons.updateButtons = function(opts) {
		var current = window.pedcache.get_count(opts);
		var nstore = window.pedcache.nstore(opts);
...
(function(pedcache, $, undefined) {
	...
	window.pedcache.clear_pedigree_data = function(opts) {
		...
		d3.selectAll(".addchild, .addpartner, .addparents, .delete, .settings")
		  .on("click", function () {
			...
			if(opt === 'settings') {
				...
				newdataset = ptree.copy_dataset(window.pedcache.current(opts));
				function onDone(opts, dataset) {
					// assign new dataset and rebuild pedigree
					opts.dataset = dataset;
					ptree.rebuild(opts);
...
function pedigree()
{
	...
	var opts = {
			...
			diseases: [	{'type': 'breast_cancer', 'colour': '#F68F35'},
				...
				{'type': 'pancreatic_cancer', 'colour': '#4289BA'},
				{'type': 'cholangiocarcinoma', 'colour': '#DDFF77'},
				...
				{'type': 'urinary_tract_cancer', 'colour': '#00FF99'},
				...
				{'type': 'blood_cancer', 'colour': '#FF5511'},
...
var abbreviation = {
	...
	"kidney_cancer":"kidney_cancer",
 	"bladder_cancer": "BLCA",
	"urinary_tract_cancer":"urinary_tract_cancer",
